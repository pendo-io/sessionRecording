...
const parseAndBuild = (json) => {
    if(json.length === 0) {
        return
    } else {
        json.forEach(element => {
            console.warn('type', element.type)
            console.warn('element', element)
            if (element.margin){
                element.margin = parseJson(element.margin)
            }
            if (element.padding){
                element.padding = parseJson(element.padding)
            }
            // do something with element
            switch (element.type) {
            case "UILayoutContainerView":
            case "UINavigationTransitionView":
            case "UIViewControllerWrapperView":
            case "UIView":
            ...
            case "UIButton":
                ...
                break;
            default:
                const defaultDiv = createDiv(element);
                mainContent.appendChild(defaultDiv);
            }
            parseAndBuild(element.views);
            });}   
        }
